...
public class WideEasy implements LevelInformation {
    ...
    public List<Velocity> initialBallVelocities() {
        ...
        double velDelta2 = 30;
        for (int i = 0; i < 5; i++) {
            vList.add(Velocity.fromAngleAndSpeed(velDelta, 300));
            vList.add(Velocity.fromAngleAndSpeed(velDelta2, 300));
            velDelta -= 10;
            velDelta2 += 10;
        }
        return vList;
    ...
    public List<Block> blocks() {
        ...
        double nextBlock = 0;
        for (int i = 0; i < 15; i++) {
            bList.add(new Block(new Point(20 + nextBlock, 200), 51, 20, blocksColor[i], 1));
            nextBlock += 50.6;
        }
        return bList;
    }
