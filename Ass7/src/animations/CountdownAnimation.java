...
public class CountdownAnimation implements Animation {
    ...
    public void doOneFrame(DrawSurface d, double dt) {
        ...
        if (this.countF >= 0) {
            if (this.countF == 0) {
                d.drawText(d.getWidth() / 2 - 8, (d.getHeight() / 2) + 35, "Go!", 50);
                d.setColor(Color.WHITE);
                d.drawText(d.getWidth() / 2 - 7, (d.getHeight() / 2) + 34, "Go!", 50);
            } else if ((this.countF > 0)) {
                d.setColor(Color.BLACK);
                d.drawText(d.getWidth() / 2, (d.getHeight() / 2) + 40, Integer.toString(this.countF), 50);
                d.setColor(Color.WHITE);
                d.drawText(d.getWidth() / 2 + 1, (d.getHeight() / 2) + 39, Integer.toString(this.countF), 50);
            }
            this.countF -= 1;
        } else {
            this.running = false;
